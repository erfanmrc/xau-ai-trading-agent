...
import { assessEntryConfluence, assessDirectionalLocation } from '@/engine/levels';
...
export function detectSP2L(c:Candle[],balance=C.balance,spread=0,precomputedContext?:MarketContext):StrategySignal{
  ...
  const spike=regime.m1Spike && end-regime.m1Spike.endIndex>=0 && end-regime.m1Spike.endIndex<=C.analysis.pullback.maxBarsAfterImpulse ? regime.m1Spike : null;
  ...
  if(context.phase!=='SPIKE') return invalid('SP2L requires a fresh active Spike phase',['Channel/transition entries are delegated to PRO_BTB or Micro-MAP']);
  ...
  const holdsBreakout=d==='LONG'?pullback.at(-1)!.close>spike.breakoutLevel+a*0.05:pullback.at(-1)!.close<spike.breakoutLevel-a*0.05;
  ...
  const spikeFvgTouched=!!spike.fvg && pullback.some(x=>x.low<=spike.fvg!.high&&x.high>=spike.fvg!.low);
  const matchedOB=context.liquidity.orderBlocks.some(x=>x.direction===d&&x.low<=entryCandidate.close+a*0.45&&x.high>=entryCandidate.close-a*0.45);
  const location=assessDirectionalLocation(c,entryCandidate.close,d,Math.max(context.liquidity.atrReference,a));
  ...
  const zoneConfirmed=!C.analysis.priceAction.requireSpikeFvgOrOrderBlock || spikeFvgTouched || matchedOB;
  if(!cleanReturn || !finalConfirm || !zoneConfirmed){
    return {strategy:'SP2L',status:'WATCH',score:clamp(48+spike.strength*0.18+(spikeFvgTouched?10:0)+(matchedOB?8:0)+(holdsBreakout?10:0)+location.score),reason:'Spike found; waiting for controlled FVG/OB pullback confirmation',reasons:[`${d} Spike: ${spike.candleCount} directional candles`,`Return depth ${returnDepth.toFixed(2)} of Leg-1`,`Breakout confirmed: ${holdsBreakout?'yes':'no'}`,`Spike FVG interaction: ${spikeFvgTouched?'yes':'no'}`,`Direction-matched OB: ${matchedOB?'yes':'no'}`,`Confirmation candle: ${finalConfirm?'yes':'no'}`,`Location: ${location.labels.join(', ')||'neutral'}`,`Liquidity/OB context: ${liq.labels.join(', ')||'none'}`],warnings:['Wait for the pullback to stabilize before Leg-2 entry'],direction:d,entry:entryCandidate.close,trigger:spike.breakoutLevel,stop:d==='LONG'?returnExtreme-a*0.08:returnExtreme+a*0.08,fvg,regime,locationScore:location.score,locationLabels:location.labels};
  }
  ...
  if(targetRR<C.singleStageMinRR || targetRR>C.singleStageMaxRR){
    return {strategy:'SP2L',status:'WATCH',score:58,reason:'SP2L target geometry is outside the single-stage 1-2R range',reasons:[`Projected RR ${targetRR.toFixed(2)}R`,`Required ${C.singleStageMinRR.toFixed(2)}-${C.singleStageMaxRR.toFixed(2)}R`],warnings:['TP remains exactly the Leg-1 projection minus spread'],direction:d,entry,trigger:spike.breakoutLevel,stop,targetLegSize:spike.legSize,targetDistance,targetRR,targetMode:'SINGLE_LEG1_MINUS_SPREAD',fvg,regime};
  }
  ...
  if(!risk.tradable) return {strategy:'SP2L',status:'INVALID',score:0,reason:'SP2L setup rejected by risk engine',reasons:risk.warnings,warnings:risk.warnings,direction:d,entry,trigger:spike.breakoutLevel,stop,risk,confluence,targetLegSize:spike.legSize,targetDistance,targetRR,targetMode:'SINGLE_LEG1_MINUS_SPREAD',fvg,regime};

  const score=clamp(45+spike.strength*0.18+confluence.score*0.35+(spikeFvgTouched?10:0)+(matchedOB?8:0)+Math.min(10,liq.score)+location.score+(regime.m1Trend.trend===d?6:0)+(regime.m5Trend.trend===d?8:0));
  const reasons=[
    ...
    `Spike FVG interaction=${spikeFvgTouched?'yes':'no'}`,
    `Direction-matched OB=${matchedOB?'yes':'no'}`,
    `Location=${location.labels.join(', ')||'neutral'}`,
    ...
    'Single-stage geometry; X2 disabled for SP2L'
  ];
  return {strategy:'SP2L',status:'VALID',score,reason:'SP2L confirmed: Spike → FVG/OB pullback → Leg-2 continuation',reasons,warnings:[],direction:d,entry,entry2:null,trigger:spike.breakoutLevel,stop,tp1:risk.takeProfit??null,tp2:risk.takeProfit??null,risk,confluence,targetLegSize:spike.legSize,targetDistance,targetRR,targetMode:'SINGLE_LEG1_MINUS_SPREAD',fvg,liquidityScore:liq.score,liquidityLabels:liq.labels,liquiditySweep:liq.sweep,orderBlock:liq.orderBlock?{...liq.orderBlock,source:'LIQUIDITY_CONTEXT'}:null,volumeProfileStatus:liquidity.volumeProfile.status,regime,locationScore:location.score,locationLabels:location.labels};
}
