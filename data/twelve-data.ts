...
export async function getXauUsdQuote(){return request<{symbol:string;close:string;datetime:string}>("/quote?symbol=XAU/USD");}
export async function getXauUsdCandles(interval="5min",outputsize=100):Promise<Candle[]>{
 const d=await request<{values?:Array<{datetime:string;open:string;high:string;low:string;close:string;volume?:string}>}>(`/time_series?symbol=XAU/USD&interval=${interval}&outputsize=${outputsize}`);
