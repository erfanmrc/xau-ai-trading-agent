  ...
  for(let i=Math.max(cfg.startIndex??60,1);i<candles.length;i++){
    ...
    for(const s of signal.signals){
      ...
      if(s.status==='VALID' && candidate){
        ...
        else {action='REJECT';rejectionReason=`Another eligible candidate selected: ${executionChosen.strategy}`;}
    ...
    const lastTrade=lastTradeIndex.get(executionChosen.strategy);
    if(lastTrade!=null && i-lastTrade<cfg.cooldownBars){
      markOpportunity(i,executionChosen.strategy,'REJECT',`Strategy cooldown active (${cfg.cooldownBars} bars)`); continue;
    }
    const lastLoss=lastLossIndex.get(executionChosen.strategy);
    if(lastLoss!=null && i-lastLoss<cfg.cooldownAfterLossBars){
      markOpportunity(i,executionChosen.strategy,'REJECT',`Post-loss cooldown active (${cfg.cooldownAfterLossBars} bars)`); continue;
    }
    ...
    if(!liveRisk.tradable||!liveRisk.lotSize){markOpportunity(i,executionChosen.strategy,'REJECT','Risk engine rejected the position size');continue;}
    if(stage==='X2' && (!liveRisk.x2Entry||!liveRisk.x2LotSize)){markOpportunity(i,executionChosen.strategy,'REJECT','Two-stage setup requires the combined 1% X2 risk plan');continue;}
    ...
    if(nowUsed+initialRisk>cfg.dailyRiskLimitPercent+1e-9){markOpportunity(i,executionChosen.strategy,'REJECT','Daily initial-risk budget would be exceeded');continue;}
    ...
    open={
      strategy:executionChosen.strategy,direction:executionChosen.direction,signalTime:c.time,entryTime,entryBar:(i+1<candles.length&&cfg.execution==='NEXT_OPEN'?i+1:i),entry,entry2,lot:liveRisk.lotSize,lot2,stop,tp,
      ...
      x2Activated:false,x2ActivationTime:null,favorableMove:0,mfeR:0,setupScore:executionChosen.score,x2TriggeredAtR:null,
