...
function enrichScore(signal:ReturnType<typeof detectSP2L>, context:ReturnType<typeof buildContext>):{score:number;h1Filter:'PASS'|'BLOCK'|'NEUTRAL';m15Relation:'CONFIRM'|'OPPOSE'|'NEUTRAL'} {
  ...
  if(!d) return {score:clamp(score),h1Filter:'NEUTRAL',m15Relation:'NEUTRAL'};
  ...
  // MTF is contextual, not a mandatory all-timeframe alignment gate.
  // H1 remains the only hard direction filter.
  if(h1Filter==='PASS') score+=10;
...
export function analyzeUnified(c:Candle[], balance=2000, spread=0):UnifiedDecision {
  ...
  const candidates:DecisionCandidate[]=valid.map((s):DecisionCandidate=>{
    ...
    const score=clamp(e.score+(conf?.score??0));
    return {
      strategy:s.strategy,
      ...
      entry:s.entry!,
      stop:s.stop!,
      signalTime:c.at(-1)?.time??new Date().toISOString(),
      h1Filter:e.h1Filter,
      m15Relation:e.m15Relation,
      confluenceScore:conf?.score??0,
      confluenceLabels:conf?.labels??[],
      reasons:[
        s.reason,
        `H1 filter: ${e.h1Filter}`,
        `M15: ${e.m15Relation}`,
        `M5 role: ${context.m5===s.direction?'aligned':context.m5==='NEUTRAL'?'neutral':'opposed context'}`,
        `M1 role: ${context.m1===s.direction?'aligned':context.m1==='NEUTRAL'?'neutral':'opposed context'}`,
        conf?.labels?.length?`Entry confluence +${conf.score}: ${conf.labels.join(', ')}`:'No round/mean level confluence',
      ]
    };
  });
  ...
  const reason=selection
    ?`One valid strategy is sufficient; selected ${selection.strategy}; H1 is ${selection.h1Filter}. M15/M5/M1 are contextual.`
    :valid.length?'Valid setup exists, but H1 blocks its direction':'No valid strategy entry yet';
  const lines=[
    'XAU AI Trading Agent',
    `Bias: ${context.bias}`,
    `MTF roles: H1 ${context.h1} (filter) | M15 ${context.m15} (structure) | M5 ${context.m5} (setup) | M1 ${context.m1} (trigger)`,
    `Session: ${context.session}`,
    ...signals.map(s=>`${s.strategy}: ${s.status}${s.direction?` ${s.direction}`:''} score=${s.score}`),
    `Valid candidates: ${candidates.length}`,
    `Eligible candidates: ${executable.length}`,
    `Selected: ${selection?`${selection.strategy} ${selection.direction} score=${selection.score}`:'NONE'}`,
    `Consensus: ${mode}${direction?` ${direction}`:''}`,
    `Reason: ${reason}`
  ];
  return {
    symbol:'XAUUSD',
    timestamp:c.at(-1)?.time??new Date().toISOString(),
    context,signals,candidates,
    selection:selection?{strategy:selection.strategy,direction:selection.direction,score:selection.score}:null,
    consensus:{direction,validCount:valid.length,alignedCount:executable.length,eligibleCount:executable.length,mode,reason},
    message:lines.join('\n')
  };
}
