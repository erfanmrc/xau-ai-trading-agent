...
import { buildLiquidityMap } from './context/liquidity';
...
export function analyzeMTF(input: MTFAnalysisInput) {
  ...
  /*
   * Zones are built from M5 first because M5 is the
   * intermediate context layer. If M5 is unavailable,
   * M1 becomes the fallback.
   */
  const zoneCandles =
    input.m5Candles.length > 0
      ? input.m5Candles
      : input.m1Candles;

  const zones = buildStrategyZones(zoneCandles);

  const liquidity = buildLiquidityMap(zoneCandles);

  /*
   * MarketContext remains backward-compatible.
   * strategyZones is attached as an optional runtime
   * property and consumed by the BTB detector.
   */
  const strategyContext = {
    ...
    macroBias:
      context.directionalBias === 'LONG'
        ? 'BULLISH' as const
        : context.directionalBias === 'SHORT'
          ? 'BEARISH' as const
          : 'NEUTRAL' as const,
  ...
  const strategies = analyzeAllStrategies(
    input.m1Candles,
    strategyContext,
    createStrategyRegistry()
  );

  const valid = strategies.activeSignals.filter(
    x =>
      context.directionalBias === null ||
      x.direction === context.directionalBias
  );

  const strategyScore =
    valid.length > 0
      ? Math.max(...valid.map(x => x.score))
      : 0;

  const qualityScore = Math.round(
    context.alignmentScore * 0.35 +
    strategyScore * 0.65
  );

  const signal =
    context.directionalBias &&
    valid.some(
      x =>
        x.status === 'VALID' &&
        x.direction === context.directionalBias
  ...
  return {
    ...
    timeframes: {
      h1: input.h1Candles.length,
      m15: input.m15Candles.length,
      m5: input.m5Candles.length,
      m1: input.m1Candles.length,
    },
    context,
    zones,
    strategies,
